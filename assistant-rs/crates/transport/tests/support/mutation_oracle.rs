//! Isolated mutation oracle: compare HTTP and every durable entity/search/ledger row.
use axum::{
    body::{Body, to_bytes},
    http::Request,
};
...
use nebula_assistant_transport::{HttpConfig, router};
...
pub async fn run(
    fixture: &Value,
    before_case: impl Fn(&Value, &Value) -> HttpConfig,
    after_case: impl Fn(&Value),
) {
    ...
    for case in fixture["cases"].as_array().unwrap() {
        ...
        let config = before_case(fixture, case);
        ...
        for change in case["expected_changes"].as_array().unwrap() {
            ...
            if let Some(saved) = after.get(id) {
                let expected_record = &change["after"];
                assert_eq!(saved["id"], expected_record["payload"]["id"]);
                assert_eq!(saved["kind"], expected_record["kind"]);
                assert_eq!(
                    saved["engagement_id"],
                    expected_record["payload"]["engagement_id"]
                );
                let expected_session = if [
                    "chat_turns",
                    "chat_schedules",
                    "chat_messages",
                    "chat_goals",
                ...
                .contains(&expected_record["kind"].as_str().unwrap())
                {
                    &expected_record["payload"]["session_id"]
                } else {
                    &expected_record["payload"]["chat_session_id"]
                };
                assert_eq!(
                    &saved["chat_session_id"], expected_session,
                    "saved session projection {id}: {}",
                    case["name"]
                );
