...
their existing tool names. The private launch directory remains isolated.

- Adapter and Core regressions: 77 passed, including authenticated Unix IPC
...
The real-agent smoke uses a controlled file-only gateway; it does not claim a
complete production chat workflow or a resumed vendor-session runtime test.
