...
## Visible relationship arrows (2026-09-09)

Journey: open Application model, show map on mobile, select connected objects and inspect a relationship. Invariant: arrows terminate outside the measured source/target cards, remain visible with long labels and retain claim-status styling. Core remains authority for relationships; URL controls filters/selection; measured DOM bounds own connector geometry. Resize/reload and keyboard/touch relationship inspection apply; no persistence, approval or deletion semantics change. Validate graph component tests and the committed dense-model real-Core production test across desktop Chromium and mobile Chromium/WebKit, including LAN origin. Physical devices are not available. This applies the broader rule that connectors must use rendered component bounds rather than guessed internal coordinates.

Scope expanded by operator screenshots: categories become collapsible dropdowns with counts; outlines render at most 20 objects per category page, and link selectors group up to 20 matches per category. The main view is a category overview until an object is selected, then a compact neighborhood of at most 100 nodes/100 connections; relationship inspection retains its source as graph focus. Relationship lists paginate at 25. Compact rows and flatter panels reduce visual clutter. Add a 2,000-object component case proving bounded DOM and deep-linked selection. This bounds rendered content, not the complete graph snapshot transferred by Core; server-side snapshot scaling remains a limitation. Browser evidence must be rerun against this expanded final UI, not inferred from the earlier arrow-only build.

Final navigation verification: 16/16 selected Vitest cases passed (graph geometry, 2,000-object outline, page workflows); TypeScript and Vite production build passed. Final dense real-Core production test passed 5/5 Chromium profiles over http://192.168.1.155:19424 (1440, 1024, mobile 320/390/430), including category pagination, selection/reload, arrow endpoint geometry, landscape and Axe checks. Evidence: /tmp/nebula-model-navigation-chromium-final. WebKit repeatedly stalled or timed out during browser/context/page creation; final WebKit and physical-device acceptance remain incomplete. Earlier test assertions were corrected to wait for the mobile map toggle and inspect a stroked SVG rather than the zero-height bounding box of a horizontal line. No server-side paging or full-graph load benchmark is claimed.

## Bounded server views and relationship recovery (2026-09-09)

Operator requests resolving full graph transfer and missing connections. UI will request a bounded server view: category counts, one 20-object category page, a selected neighborhood capped at 100 objects/relationships, and a 25-relationship page. URL owns selection/filter/page; Core owns counts and revision. Selected objects and edge endpoints must remain inspectable even outside the outline page. Link selectors use project-scoped remote search, not partial-view arrays. Refresh/filter races must not mix contexts. Keep full graph export backward compatible, but remove it from the browser's normal read path. Tests must check pagination, endpoint bounds, selected deep links, cross-project isolation, and real browser requests.

Live diagnosis: agent relationship transactions using `hosts` and `exposes` failed schema validation, then it continued adding objects. Improve discoverability and actionable validation recovery; require explicit relationship consideration during browsing. Repair existing connections only from stored source observations/captures, using documented project extensions where the built-in vocabulary does not express observed URL/navigation relationships. Branding alone does not establish backend hosting. Record all repairs as ordinary revisioned, evidence-bearing transactions; do not invent a hosting relationship.

Bounded-view verification: selected backend 5/5 and component 17/17 passed; Ruff and diff check passed; TypeScript/Vite production build passed. The committed real-Core dense journey passed 5/5 Chromium profiles (1440/1024 and emulated Android 320/390/430) on LAN http://192.168.1.155:19427, including remote endpoint search, saved linking, pagination, reload, SVG geometry and Axe. Evidence: /tmp/nebula-model-view-chromium-final. WebKit 320/390 timed out during setup and the 430 run did not finish before the 210-second process bound; evidence /tmp/nebula-model-view-webkit. Physical devices unavailable. Acceptance remains partially verified. Core still reads its whole graph document internally; bounded transfer is not indexed storage.

Existing-data repair: verified stored observation URLs and SHA-256 capture artifacts, then rehearsed an idempotent transaction on a SQLite online backup. Live revision 12 -> 13 retains all 14 objects and adds 47 observed relationships: 10 custom.addressed_by_domain and 37 custom.links_to_endpoint. No service-hosting claim was inferred from branding. Saved proposal, prior graph and receipt are under /home/agent/.local/share/nebula/v3/backups/relationship-repair-20260909. Agent discovery/recovery is tested across browser/harness brokers; future live agent compliance is not mechanically guaranteed by instruction tests.

WebKit recovery: the same committed dense graph journey subsequently passed 3/3 emulated iPhone profiles (320/390/430) against the final production bundle on http://192.168.1.155:19434. Evidence: /tmp/nebula-model-webkit-final. This closes the automated WebKit gate; physical-device acceptance remains unavailable.
