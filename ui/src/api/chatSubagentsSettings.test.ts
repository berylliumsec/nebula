...
it("saves main-agent messaging and includes it only when enabled on turns", async () => {
  const wire = {
    ...
    provider_profile_id: "provider", model: "model-a", revision: 4,
    created_at: "2026-09-21T10:00:00Z", updated_at: "2026-09-21T10:00:00Z",
    metadata: { allow_agent_messaging: true },
  };
  const fetchMock = vi.fn<typeof fetch>().mockResolvedValue(new Response(JSON.stringify(wire), { status: 200 }));
  const client = new ApiClient({ baseUrl: "http://127.0.0.1:8765", fetch: fetchMock });

  const updated = await client.updateChatSessionAssistantSettings("session-1", {
    allowAgentMessaging: true, expectedRevision: 3,
  });

  expect(updated).toMatchObject({ allowAgentMessaging: true });
  expect(JSON.parse(String(fetchMock.mock.calls[0][1]?.body))).toMatchObject({
    allow_agent_messaging: true, expected_revision: 3,
  });
  const enabled = chatRequestBody({ messages: [{ role: "user", content: "Coordinate" }], allowAgentMessaging: true }, true);
  expect(enabled.allow_agent_messaging).toBe(true);
  expect(chatRequestBody({ messages: [{ role: "user", content: "Work alone" }] }, true)).not.toHaveProperty("allow_agent_messaging");
});
