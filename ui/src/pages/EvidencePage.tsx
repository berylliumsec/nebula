  ...
  return (
    <div className="page evidence-page">
      ...
      <section className="panel data-panel evidence-panel">
        ...
        <div className="artifact-grid">{visibleItems.map((item) => { const Icon = evidenceIcon(item); return <article className="artifact-card" key={item.id}><div className="artifact-preview"><Icon size={31} strokeWidth={1.4} /><span>{item.evidenceType.replaceAll("_", " ")}</span></div><div className="artifact-body"><h3>{item.title}</h3><p>{formatSize(item.metadata.size)} · <code>{item.sha256 ? `${item.sha256.slice(0, 10)}…` : "hash unavailable"}</code></p><dl><div><dt>Captured by</dt><dd>{operatorLabel(item.capturedBy)}</dd></div><div><dt>Assets</dt><dd>{item.assetIds.length || "Not linked"}</dd></div></dl></div><footer><span><LockKeyhole size={13} /> {item.sha256 ? "Hash recorded" : "Metadata only"}</span><button className="text-link" type="button" disabled={previewMode || !item.artifactId || busyId === item.id} onClick={() => void download(item)}>{busyId === item.id ? "Downloading…" : "Download"}</button></footer></article>; })}{visibleItems.length === 0 && <div className="empty-state compact"><LockKeyhole size={23} /><strong>{query ? "No matching evidence" : "No evidence loaded"}</strong><p>{query ? "Try another artifact name, source, or hash." : "Upload an artifact to create an immutable evidence record."}</p></div>}</div>
      </section>
