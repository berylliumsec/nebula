...
export function AssetsPage() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      void logCaughtDiagnostic("interface.assets_page.caught_failure_01", "A handled interface operation failed.", createError, "assets_page");
  ...
  return (
    <div className="page assets-page">
      ...
      <section className="panel data-panel">
        ...
        <div className="table-scroll" tabIndex={0} role="region" aria-label="Assets table">
      ...
      {adding && <ModalSurface as="form" className="provider-dialog resource-dialog" labelledBy="asset-dialog-title" onClose={closeAssetDialog} onSubmit={(event) => void submit(event)}><header><div><small>Project asset</small><h2 id="asset-dialog-title">Add asset</h2></div><button className="icon-button subtle" type="button" aria-label="Close asset dialog" disabled={saving} onClick={closeAssetDialog}><X size={17} /></button></header><label>Name<input required autoFocus value={name} onChange={(event) => setName(event.target.value)} /></label><label>Kind<select value={assetKind} onChange={(event) => setAssetKind(event.target.value as AssetSummary["kind"])}>{(["host", "domain", "url", "cloud", "repository", "other"] as const).map((value) => <option value={value} key={value}>{value}</option>)}</select></label><div className="resource-form-grid"><label>Address<input value={address} placeholder="IP, CIDR, or URL" onChange={(event) => setAddress(event.target.value)} /></label><label>Hostname<input value={hostname} onChange={(event) => setHostname(event.target.value)} /></label><label>Criticality<select value={criticality} onChange={(event) => setCriticality(event.target.value as AssetSummary["criticality"])}>{(["critical", "high", "medium", "low", "info"] as const).map((value) => <option value={value} key={value}>{value}</option>)}</select></label><label>Exposure<select value={assetExposure} onChange={(event) => setAssetExposure(event.target.value as AssetSummary["exposure"])}>{(["unknown", "external", "internal"] as const).map((value) => <option value={value} key={value}>{value}</option>)}</select></label></div><label>Tags<input value={tags} placeholder="production, api (comma-separated)" onChange={(event) => setTags(event.target.value)} /></label>{error && <DiagnosticErrorNotice error={error} fallback="The asset could not be added." compact />}<footer><button className="button secondary" type="button" disabled={saving} onClick={closeAssetDialog}>Cancel</button><button className="button primary" type="submit" disabled={saving || !name.trim()}>{saving ? "Adding…" : "Add asset"}</button></footer></ModalSurface>}

      {missingResourceId && <div className="workspace-state-banner degraded" role="alert"><span><strong>Asset unavailable</strong><small>The linked asset was deleted, is inaccessible, or belongs to another project.</small></span><button className="button quiet" type="button" onClick={closeResource}>Return to assets</button></div>}
      {selected && <aside className="resource-inspector" role="complementary" aria-labelledby="asset-detail-title"><header><div><small>{selected.kind} · {selected.exposure}</small><h2 id="asset-detail-title">{selected.displayName}</h2></div><button className="icon-button subtle" type="button" aria-label="Close asset details" onClick={closeResource}><X size={17} /></button></header><dl className="resource-details"><div><dt>Address</dt><dd>{selected.address || "Not recorded"}</dd></div><div><dt>Hostname</dt><dd>{selected.hostname || "Not recorded"}</dd></div><div><dt>Criticality</dt><dd>{selected.criticality}</dd></div><div><dt>Services</dt><dd>{selected.serviceCount ?? "Not recorded"}</dd></div><div><dt>Findings</dt><dd>{findingCount(selected.id)}</dd></div><div><dt>Last observed</dt><dd>{displayTime(selected.lastSeenAt)}</dd></div></dl><div className="scope-chip-list">{selected.tags.length ? selected.tags.map((tag) => <span key={tag}>{tag}</span>) : <span>No tags</span>}</div>{engagement && <ResourceRelationsPanel resource={{ projectId: engagement.id, kind: "asset", id: selected.id }} />}</aside>}
    </div>
  );
}
