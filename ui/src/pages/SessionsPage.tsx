...
export function SessionsPage() {
  ...
  const harnessMcpCount = selectedMcpIds.length
    || harnessSessions.find((item) => item.id === harnessSessionId)?.mcpServerIds.length
    || 0;
  const harnessIsReady = !sending
    && !harnessActivity?.busy
    && (visibleHarnessProgress?.phase === "ready" || visibleHarnessProgress?.phase === "complete");
