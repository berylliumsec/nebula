...
type SessionView = "chat" | "terminal" | "browser" | "missions" | "activity" | "workspace" | "notes";
...
export function SessionsPage() {
  ...
  const initialView = requestedView === "chat" || requestedView === "terminal" || requestedView === "browser" || requestedView === "missions" || requestedView === "activity" || requestedView === "workspace" || requestedView === "notes"
    ? requestedView
  ...
  const [view, setViewState] = useState<SessionView>(initialView === "chat" || initialView === "browser" || initialView === "missions" || initialView === "activity" || initialView === "workspace" || initialView === "notes" ? initialView : "terminal");
  ...
  useEffect(() => {
    ...
    if (next === "chat" || next === "terminal" || next === "browser" || next === "missions" || next === "activity" || next === "workspace" || next === "notes") {
      setViewState(next);
      localStorage.setItem("nebula.workbench.view", next);
    }
  }, [requestedView]);
  ...
  return (
    <div className="page sessions-page">
      <PageHeader
        title="Workbench"
        description="Start in Terminal, browse a target, ask the assistant, or open your project files."
      ...
      <div className={`session-layout ${view}${mobileListOpen ? " mobile-list-open" : ""}${view === "chat" && conversationPanelExpanded ? " conversation-panel-expanded" : ""}`}>
        ...
        <section className="session-workspace">
          ...
          ) : view === "terminal" || (view === "browser" && engagement) ? null : view === "missions" && api && engagement ? (
            <AgentsPage embedded />
