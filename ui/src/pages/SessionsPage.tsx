...
import { McpServerChoices } from "../components/McpServerChoices";
...
export function SessionsPage() {
  ...
  // The Agent view floats over the conversation unless the operator docked it
  // in the details; that choice is remembered on this device.
  const [agentView, setAgentView] = useState<"closed" | "floating" | "minimized">("closed");
  const [agentViewPlacement, setAgentViewPlacement] = useState<AgentViewPlacement>(readPlacement);
  const agentViewButtonRef = useRef<HTMLButtonElement>(null);
  const [agentViewJustDocked, setAgentViewJustDocked] = useState(false);
  const agentViewPopOutFocused = useCallback(() => setAgentViewJustDocked(false), []);
  const agentViewDocked = sessionInspectorOpen && drawerTab === "visuals";
  const { unseen: publishedUnseen } = useUnseenCount(
    publishedResults.items,
    agentViewDocked || agentView === "floating",
  );
  // One view at a time: opening the docked tab takes the floating one's place.
  useEffect(() => {
    if (agentViewDocked) setAgentView("closed");
  }, [agentViewDocked]);
  const placeAgentView = (placement: AgentViewPlacement) => {
    writePlacement(placement);
    setAgentViewPlacement(placement);
  };
  ...
  useEffect(() => {
    // A provider-backed conversation has no saved harness selection. When the
    // operator switches it to a harness, normalize the newly selected runtime
    // just as we do for an unsaved chat. Keep an existing harness conversation's
    // saved model untouched while its history is loading.
    if (runtimeKind !== "harness" || (sessionId && activeChatSession?.backend === "harness")) return;
    ...
    <div className={`page sessions-page${view === "chat" ? " chat-active" : ""}${screenFitViews.has(view) ? " screen-fit" : ""}${fullScreen ? " full-screen" : ""}`}>
      ...
      <div className={`session-layout ${view}${mobileListOpen ? " mobile-list-open" : ""}${view === "chat" && conversationPanelOpen ? " conversation-panel-open" : ""}${view === "chat" && sessionInspectorOpen ? " inspector-open" : ""}`}>
        ...
        {(view === "chat" || view === "browser") && agentView !== "closed" && api && engagement && sessionId && <AgentViewPanel
          ...
          onDock={() => { placeAgentView("docked"); setAgentView("closed"); setAgentViewJustDocked(true); updateSearchParams(next => {next.set("drawer", "visuals");}); }}
        />}
