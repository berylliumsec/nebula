...
describe("web search runtime settings", () => {
  ...
  it("shows a ready runtime with its pinned image and loopback address", async () => {
    mount();
    expect(await screen.findByText(/searxng\/searxng/)).toBeInTheDocument();
  ...
  it("states there is no key to manage", async () => {
    mount();
    await screen.findByText(/searxng\/searxng/);
  ...
  it("explains a missing container runtime instead of offering dead controls", async () => {
    ...
    mount();
    expect(await screen.findByText(/needs an enabled, verified container runtime/)).toBeInTheDocument();
    expect(screen.getByRole("status")).toHaveTextContent("No container runtime");
  });
  ...
  it("reports a failed test with its reason", async () => {
    ...
    mount();
    expect(await screen.findByText(/the search runtime is not installed/)).toBeInTheDocument();
  });
...
describe("runtime health and digest helpers", () => {
  it("maps each container state to an operator-readable status", () => {
    expect(runtimeHealth(undefined).label).toBe("Checking\u2026");
