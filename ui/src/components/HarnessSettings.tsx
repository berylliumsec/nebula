...
export function HarnessSettings() {
  ...
  const kind: HarnessProfile["kind"] = "codex_app_server";
    ...
    try {
      ...
      const payload = {
        ...
        native_capabilities: {
          ...
          image_generation: nativeImageGeneration,
          skills: nativeSkills,
  ...
  return <>
    <section className="settings-section" id="harness-settings">
      <div className="section-heading"><div><h2>Agent harnesses</h2><p>Codex runtimes</p></div><button className="button primary" type="button" disabled={previewMode} onClick={() => openHarness()}><Plus size={16} /> Add harness</button></div>
      ...
      {harnesses.length ? <div className="provider-grid">{harnesses.map((profile) => <article className="panel provider-card integration-card" key={profile.id}>
        <header className="integration-card-heading"><span className={`status-dot ${profile.enabled ? profile.healthy ? "healthy" : "warning" : "unavailable"}`} /><div><small>Codex App Server</small><h3>{profile.name}</h3></div></header>
        ...
        <dl className="integration-card-facts"><div><dt>Model</dt><dd>{profile.defaultModel ?? "Selected per session"}</dd></div><div><dt>Auth</dt><dd>{profile.authMode === "existing_session" ? "Existing local sign-in" : "Secret-backed · configured"}</dd></div><div><dt>Privacy</dt><dd>{profile.localOnly ? "Local runtime" : profile.permitsSensitiveData ? "Cloud data allowed with confirmation" : "Text only"}</dd></div><div><dt>Version</dt><dd title={profile.version}>{profile.version ?? "Not checked"}</dd></div></dl>
