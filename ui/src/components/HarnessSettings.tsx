  ...
  return <>
    <section className="settings-section" id="harness-settings">
      ...
      {harnesses.length ? <div className="provider-grid">{harnesses.map((profile) => <article className="panel provider-card" key={profile.id}>
        ...
        <footer><button className="button quiet" type="button" disabled={busy === profile.id} onClick={() => { setBusy(profile.id); void api?.checkHarness(profile.id).then(reload).catch((actionError) => setError(actionError instanceof Error ? actionError.message : "Health check failed.")).finally(() => setBusy(undefined)); }}><RefreshCw className={busy === profile.id ? "spin" : undefined} size={14} /> Check</button><button className="icon-button subtle" aria-label={`Edit ${profile.name}`} type="button" onClick={() => openHarness(profile)}><Pencil size={14} /></button><button className="button quiet" type="button" disabled={busy === profile.id} onClick={() => void updateHarness(profile, { enabled: !profile.enabled })}>{profile.enabled ? "Disable" : "Enable"}</button><button className="icon-button subtle" aria-label={`Delete ${profile.name}`} type="button" disabled={busy === profile.id} onClick={() => { setBusy(profile.id); void api?.deleteHarness(profile.id, profile.revision).then(reload).catch((actionError) => setError(actionError instanceof Error ? actionError.message : "Delete failed.")).finally(() => setBusy(undefined)); }}><Trash2 size={14} /></button></footer>
    ...
    <section className="settings-section" id="mcp-settings">
      <div className="section-heading"><div><h2>MCP servers</h2><p>Nebula-owned profiles frozen into each new harness session.</p></div><button className="button primary" type="button" disabled={previewMode} onClick={() => openMcp()}><Plus size={16} /> Add MCP server</button></div>
