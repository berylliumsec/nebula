...
export function ChatRecordedContext({api, sessionId, onMessage}: {api: ApiClient; sessionId: string; onMessage: (id: string) => void}) {
  ...
  // Offsets of the newer pages the operator paged away from, so "Older context" has a way back.
  const [newer, setNewer] = useState<number[]>([]);
  const [error, setError] = useState<string>();
  const older = () => { if (next === null) return; setNewer(current => [...current, offset]); setOffset(next); };
  const newerPage = () => { const previous = newer.at(-1); if (previous === undefined) return; setNewer(current => current.slice(0, -1)); setOffset(previous); };
