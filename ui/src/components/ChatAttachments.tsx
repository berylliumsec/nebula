...
export function ChatAttachments({api, projectId, onAttach, onImages, imagesEnabled}: {api: ApiClient; projectId: string; onAttach: (value: NebulaDraftRequest) => void; onImages: () => void; imagesEnabled: boolean}) {
  ...
  return <><button className="button quiet" type="button" onClick={() => setOpen(true)}>Attach files</button>{open && <ModalSurface as="section" className="provider-dialog assistant-attachment-dialog" labelledBy="chat-attachment-title" onClose={() => setOpen(false)}><header><h2 id="chat-attachment-title">Attach to next message</h2><button type="button" className="button quiet" onClick={() => setOpen(false)}>Close attachments</button></header><p>Preview the exact text before attaching it. Documents upload to this project’s knowledge library.</p><nav>{imagesEnabled && <button type="button" className="button quiet" disabled={busy} onClick={() => {setOpen(false); onImages();}}>Images from this device</button>}<button type="button" className="button quiet" disabled={busy} onClick={() => input.current?.click()}>Document from this device</button><button type="button" className="button quiet" disabled={busy} onClick={() => void browse()}>Browse project files</button><button type="button" className="button quiet" disabled={busy} onClick={() => void run(async () => {setListing(undefined); setPreview(undefined); setSources((await api.listKnowledgeSources(projectId)).items);})}>Choose knowledge source</button></nav>
