      ...
      <div className="execution-history-layout">
        ...
        <section className={`execution-detail${selected ? "" : " is-empty"}`}>
          {selected ? <>
            ...
            <dl><div><dt>Image</dt><dd><code>{selected.runtime.image}</code></dd></div><div><dt>Runtime digest</dt><dd><code>{selected.runtime.runtimeDigest}</code></dd></div><div><dt>Network</dt><dd>{selected.network.mode === "none" ? "Offline" : selected.network.mode === "unrestricted" ? "Unrestricted outbound" : `${selected.network.target} · ${selected.network.ports.join(", ")}`}</dd></div><div><dt>Policy</dt><dd>{selected.policyDecision}{selected.errorCode ? ` · ${selected.errorCode}` : ""}</dd></div></dl>
