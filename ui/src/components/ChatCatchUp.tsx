...
export function ChatCatchUp({api, sessionId, ready, atLatest, onMessage, onPending, onTurn}: {api: ApiClient; sessionId: string; ready: boolean; atLatest: boolean; onMessage: (id: string) => void; onPending: () => void; onTurn: (id: string) => void}) {
  ...
  useEffect(() => {
    ...
    const report = (e: unknown) => {if (!controller.signal.aborted) {void logCaughtDiagnostic("interface.assistant_chat.cursor_failed", "The device read cursor could not be synchronized.", e, "assistant_chat"); setError("Read state could not be synchronized. Reload catch-up to retry; pending actions remain visible.");}};
    let device: string;
  ...
  }, [api, sessionId, ready, refresh]);
  return <>{error && <div className="chat-recovery-notice" role="status"><p>{error}</p><button className="button quiet" type="button" onClick={() => {setError(undefined); setRefresh(value => value + 1);}}>Reload catch-up</button></div>}{card && card.items.length > 0 && <section className="chat-catch-up" aria-label="Catch up on this conversation"><header><strong>Since you last read this conversation</strong><button className="button quiet" type="button" onClick={() => void acknowledgeRef.current?.(card)}>Dismiss catch-up</button></header>{card.truncated && <p>Showing up to 50 recent updates. Open the transcript for the full history.</p>}<ul>{card.items.map(item => <li key={item.id}><button className="button quiet" type="button" onClick={() => item.kind === "failure" && item.turn_id ? onTurn(item.turn_id) : item.message_id ? onMessage(item.message_id) : item.turn_id ? onTurn(item.turn_id) : onPending()}><strong>{item.kind}</strong><span>{item.text}</span></button></li>)}</ul></section>}{pending.length > 0 && <div className="chat-recovery-notice" role="status"><strong>{pending.length} pending operator action(s)</strong><button className="button quiet" type="button" onClick={onPending}>Review pending actions</button></div>}</>;
}
