...
export function DevicePairingSettings({ api, disabled, onCurrentDeviceRevoked }: DevicePairingSettingsProps) {
  ...
  return <section className="panel device-pairing-panel" id="device-pairing-settings">
    ...
    <div className="operator-profile-list">{devices.map((device) => <article key={device.id}><span className={`operator-profile-avatar${device.healthy ? " healthy" : ""}`}><Smartphone size={16} /></span><div><h3>{device.name}{device.current ? " · this device" : ""}</h3><p>{device.healthy ? "Connected now" : device.heartbeatAt ? `Last heartbeat ${new Date(device.heartbeatAt).toLocaleString()}` : `Last used ${new Date(device.lastUsedAt).toLocaleString()}`}{device.platform ? ` · ${device.platform}${device.appVersion ? ` ${device.appVersion}` : ""}` : ""}</p>{Boolean(device.capabilities?.length) && <small>{device.capabilities!.join(" · ")}</small>}</div><button className="icon-button subtle" type="button" disabled={disabled || Boolean(revokingId)} aria-label={`${device.current ? "Unpair" : "Revoke"} ${device.name}`} onClick={() => void revokeDevice(device)}>{revokingId === device.id ? <LoaderCircle className="spin" size={14} /> : <Trash2 size={14} />}</button></article>)}</div>
