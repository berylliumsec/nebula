...
const implicitEnvironmentTools = new Set([
  "environment.shell_local",
  "environment.shell_network",
]);

export function EngagementPolicySettings() {
  const { activeOperator, api, coreState, engagement, previewMode, providers } =
    useWorkspace();
  ...
  const [grantRiskClasses, setGrantRiskClasses] = useState<string[]>([
    "exploitation",
  ]);
  ...
  const applyAssignment = (next: EngagementToolAssignment) => {
    setAssignment(next);
    setSelectedDigest(next.manifestDigest ?? "");
    setSelectedTools(
      next.toolNames.filter((name) => !implicitEnvironmentTools.has(name)),
    );
    setAssignmentEnabled(next.enabled);
  };

  const load = useCallback(async () => {
    if (!api || coreState !== "online" || !engagement) return;
    setError(undefined);
    const [scopeResult, assignmentResult, packsResult, toolsResult] =
    ...
    if (scopeResult.status === "fulfilled") {
      setScopeAvailable(true);
      applyScope(scopeResult.value);
    } else if (featureMissing(scopeResult.reason)) {
      setScopeAvailable(false);
      applyScope(emptyScope(engagement.id));
    } else
      setError(
        scopeResult.reason instanceof Error
          ? scopeResult.reason.message
          : "Could not load the engagement scope.",
      );
    if (assignmentResult.status === "fulfilled") {
      setAssignmentAvailable(true);
      setAssignments(assignmentResult.value);
    } else if (featureMissing(assignmentResult.reason)) {
      ...
      setError(
        assignmentResult.reason instanceof Error
          ...
          : "Could not load tool assignments.",
      );
  ...
  useEffect(() => {
    void load();
  }, [load, toolPackRevision]);

  const readyPacks = useMemo(
    () => packs.filter((pack) => pack.status === "ready"),
    [packs],
  );
  const selectedPack = readyPacks.find(
    (pack) => pack.manifestDigest === selectedDigest,
  );
  const readyDigests = useMemo(
    () => new Set(readyPacks.map((pack) => pack.manifestDigest)),
    [readyPacks],
  );
  const packTools = selectedPack
    ? tools.filter(
        (tool) =>
          tool.packManifestDigest === selectedDigest ||
          tool.packId === selectedPack.id,
      )
    : [];
  const configurableTools = packTools.filter(
    (tool) => !implicitEnvironmentTools.has(tool.name),
  );
  const grantableToolNames = useMemo(
    () =>
      [
        ...new Set(
          assignments
            .filter(
              (item) =>
                item.enabled &&
                item.manifestDigest !== undefined &&
                readyDigests.has(item.manifestDigest),
            )
            .flatMap((item) => item.toolNames)
            .filter((name) => !implicitEnvironmentTools.has(name)),
        ),
      ].sort(),
    [assignments, readyDigests],
  );
    ...
    const assignedReadyDigest = assignments
      ...
      .find(
        (digest): digest is string =>
          digest !== undefined && readyDigests.has(digest),
      );
    setSelectedDigest(
      assignedReadyDigest ?? readyPacks[0]?.manifestDigest ?? "",
    );
  ...
  useEffect(() => {
    ...
    applyAssignment(
      assignments.find((item) => item.manifestDigest === selectedDigest) ?? {
        ...emptyAssignment(engagement.id),
  ...
  const saveScope = async (event: FormEvent) => {
    ...
    try {
      ...
      void logCaughtDiagnostic(
        "interface.engagement_policy_settings.caught_failure_01",
        "A handled interface operation failed.",
        saveError,
        "engagement_policy_settings",
      );
      setError(
        saveError instanceof Error
          ? saveError.message
          : "Could not save the engagement scope.",
      );
    } finally {
      setSaving(undefined);
    }
  ...
  const saveAssignment = async (event: FormEvent) => {
    ...
    setError(undefined);
    try {
      const next = await api.updateEngagementToolAssignment(engagement.id, {
        manifestDigest: selectedDigest,
      ...
      setAssignments((current) => [
        next,
        ...current.filter(
          (item) => item.manifestDigest !== next.manifestDigest,
        ),
      ]);
      applyAssignment(next);
    } catch (saveError) {
      void logCaughtDiagnostic(
        "interface.engagement_policy_settings.caught_failure_02",
        "A handled interface operation failed.",
        saveError,
        "engagement_policy_settings",
      );
      setError(
        saveError instanceof Error
          ? saveError.message
          : "Could not save the tool assignment.",
      );
    } finally {
      setSaving(undefined);
    }
  ...
  const addGrant = () => {
    ...
    if (!expiresAt || expiresAt <= new Date().toISOString()) {
      setGrantError("A mission grant needs a future expiration time.");
      return;
    }
    ...
    setGrants((current) => [
      ...current,
      {
        riskClasses: grantRiskClasses,
        toolNames: grantTools,
        targets: lines(grantTargets),
        grantedAt: now,
        expiresAt,
        grantedBy: activeOperator?.id ?? "local-operator",
      },
    ]);
    setGrantOpen(false);
    setGrantRiskClasses(["exploitation"]);
    setGrantTargets("");
    setGrantTools([]);
    setGrantExpires("");
  ...
  if (!engagement)
    return (
      <section className="settings-section" id="engagement-policy-settings">
        <div className="empty-state compact">
          <ShieldAlert size={22} />
          <strong>No engagement selected</strong>
          <p>
            Select an engagement before defining its scope or assigning tools.
          </p>
        </div>
      </section>
    );
    ...
    <section className="settings-section" id="engagement-policy-settings">
      <div className="section-heading">
        <div>
          ...
          <p>
            Exact rules of engagement and environment access for{" "}
            {engagement.name}.
          </p>
        </div>
        <span className="policy-revision">
          Scope revision {scope?.revision ?? 0}
        </span>
      </div>
      {error && (
      ...
      <div className="policy-editor-grid">
        <form
          ...
          onSubmit={(event) => void saveScope(event)}
        >
          <header className="panel-header compact">
            <div>
              <h3>Rules of engagement</h3>
              <p>Core enforces these values before any network request.</p>
            </div>
            <ShieldCheck size={18} />
          </header>
          {!scopeAvailable ? (
            <div className="feature-unavailable" role="status">
              <ShieldAlert size={21} />
              <div>
                <strong>Scope editing is unavailable</strong>
                <p>This Core build keeps all missions analysis-only.</p>
              </div>
            </div>
          ) : (
            <div className="policy-form-body">
              <button
                className="button secondary scope-import-trigger"
                type="button"
                disabled={previewMode || !api || !scope}
                onClick={() => setScopeImportOpen(true)}
              >
                <FileUp size={14} /> Import targets from document
              </button>
              ...
              <label>
                ...
                <textarea
                  rows={2}
                  value={allowedUrls}
                  placeholder="https://app.example.test/api"
                  onChange={(event) => setAllowedUrls(event.target.value)}
                />
              </label>
              <label>
                Allowed ports
              ...
              <div className="resource-form-grid">
                ...
                <label className="provider-consent">
                  ...
                  <span>
                    ...
                    <small>
                      Prevent cloud providers from receiving engagement data.
                    </small>
                  </span>
                </label>
              </div>
              <section className="mission-grants">
                <header>
                  <div>
                    <strong>Optional high-risk grants</strong>
                    <small>
                      Ordinary in-scope Toolbox scans do not need a grant;
                      invasive extension capabilities still do.
                    </small>
                  </div>
                  <button
                ...
                {grants.length ? (
                  grants.map((grant, index) => (
                    <article key={`${grant.grantedAt}-${index}`}>
                      <Clock3 size={14} />
                      <div>
                        <strong>
                          {grant.riskClasses.join(", ") || "No risk classes"}
                        </strong>
                        <small>
                          {grant.toolNames.join(", ") ||
                            "All assigned capabilities"}{" "}
                          · expires{" "}
                      ...
                      <button
                        ...
                        type="button"
                        aria-label="Remove mission grant"
                        onClick={() =>
                          setGrants((current) =>
                            current.filter(
                  ...
                  <p>No optional high-risk grants configured.</p>
                )}
              </section>
              <footer>
                <span>Saving creates a revision-safe policy update.</span>
                <button
                  className="button primary"
                  type="submit"
                  disabled={previewMode || saving === "scope"}
                >
                  <Save size={14} />{" "}
                  {saving === "scope" ? "Saving…" : "Save scope"}
                </button>
              </footer>
            </div>
          )}
        ...
        <form
          className="panel policy-form assignment-form"
          onSubmit={(event) => void saveAssignment(event)}
        >
          <header className="panel-header compact">
            <div>
              <h3>Execution-environment assignment</h3>
              <p>
                Loaded local environments are enabled automatically; use this
                control to narrow or disable access.
              </p>
            </div>
            <Wrench size={18} />
          </header>
          {!assignmentAvailable ? (
            <div className="feature-unavailable" role="status">
              <Wrench size={21} />
              <div>
                <strong>Environment assignments are unavailable</strong>
                <p>Missions remain analysis-only.</p>
              </div>
            </div>
          ) : (
            <div className="policy-form-body">
              <label>
                ...
                <select
                  aria-label="Assigned execution environment"
                  value={selectedDigest}
                  disabled={!readyPacks.length}
                  onChange={(event) => {
                    setSelectedDigest(event.target.value);
                    setSelectedTools([]);
                  }}
                >
                  {readyPacks.length ? (
                    readyPacks.map((pack) => (
                      <option value={pack.manifestDigest} key={pack.id}>
                        {pack.publisher}/{pack.name} · {pack.version}
                      </option>
                    ))
                  ) : (
                    <option value="">No verified environments</option>
                  )}
                </select>
              </label>
              {selectedDigest && (
                <code className="digest-lock" title={selectedDigest}>
                  {selectedDigest}
                </code>
              )}
              {selectedDigest && (
                <p>
                  Local shell access and scoped-network shell access are
                  included automatically when declared by this environment.
                </p>
              )}
              <fieldset className="resource-checklist">
                <legend>Optional environment capabilities</legend>
                {configurableTools.length ? (
                  ...
                  <p>
                    {readyPacks.length
                      ? "No optional capabilities are exposed by this environment."
                      : "Install and verify an environment first."}
                  </p>
                )}
              </fieldset>
              <label className="provider-consent">
                ...
                />
                <span>
                  <strong>Enable this assignment</strong>
                  <small>
                    The exact image manifest digest, automatic shell access, and
                    checked optional capabilities become available.
                  </small>
                </span>
              </label>
              <footer>
                <span>Assignment revision {assignment?.revision ?? 0}</span>
                <button
                  className="button primary"
                  type="submit"
                  disabled={
                    previewMode || !selectedDigest || saving === "assignment"
                  }
                >
                  <Save size={14} />{" "}
      ...
      {grantOpen && (
        <div className="dialog-backdrop">
          ...
          >
            <header>
              <div>
                <small>Explicit authorization</small>
                <h2 id="grant-dialog-title">Add high-risk grant</h2>
              </div>
              <button
                className="icon-button subtle"
                type="button"
                aria-label="Close mission grant dialog"
            ...
            <fieldset className="resource-checklist">
              ...
              {grantRiskOptions.map((risk) => (
                <label key={risk}>
                  <input
                    type="checkbox"
                    checked={grantRiskClasses.includes(risk)}
                    onChange={(event) =>
                      setGrantRiskClasses((current) =>
                        event.target.checked
                          ? [...current, risk]
                  ...
                  />
                  <span>{risk.replaceAll("_", " ")}</span>
                </label>
              ))}
            </fieldset>
            <fieldset className="resource-checklist">
              <legend>Capabilities</legend>
              {grantableToolNames.map((name) => (
                <label key={name}>
                  <input
                    type="checkbox"
                    checked={grantTools.includes(name)}
                    onChange={(event) =>
                      setGrantTools((current) =>
                        event.target.checked
                          ? [...current, name]
              ...
              {!grantableToolNames.length && (
                <p>Save and enable an environment assignment first.</p>
              )}
            </fieldset>
            <label>
              Targets
              <textarea
                ...
                onChange={(event) => setGrantTargets(event.target.value)}
              />
            </label>
            <label>
              Expires
              <input
                type="datetime-local"
                value={grantExpires}
                onChange={(event) => setGrantExpires(event.target.value)}
              />
            </label>
            {grantError && (
            ...
            <p className="provider-dialog-note">
              The grant is attributed to{" "}
              {activeOperator?.displayName ?? "the active local operator"}. It
              cannot expand the engagement scope.
            </p>
            <footer>
              <button
