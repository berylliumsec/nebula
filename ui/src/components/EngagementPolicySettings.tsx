...
function emptyAssignment(engagementId: string): EngagementToolAssignment {
  return { engagementId, toolNames: [], enabled: false, revision: 0 };
}
...
export function EngagementPolicySettings() {
  ...
  const [grantRiskClasses, setGrantRiskClasses] = useState<string[]>(["active_scan"]);
  ...
  const applyAssignment = (next: EngagementToolAssignment) => {
    setAssignment(next); setSelectedDigest(next.manifestDigest ?? ""); setSelectedTools(next.toolNames); setAssignmentEnabled(next.enabled);
  };
  ...
  const addGrant = () => {
    ...
    setGrantOpen(false); setGrantRiskClasses(["active_scan"]); setGrantTargets(""); setGrantTools([]); setGrantExpires(""); setGrantError(undefined); setError(undefined);
  };
  ...
  return <section className="settings-section" id="engagement-policy-settings"><div className="section-heading"><div><h2>Engagement policy</h2><p>Exact rules of engagement and tool grants for {engagement.name}.</p></div><span className="policy-revision">Scope revision {scope?.revision ?? 0}</span></div>{error && <div className="knowledge-status error" role="alert">{error}</div>}<div className="policy-editor-grid"><form className="panel policy-form" onSubmit={(event) => void saveScope(event)}><header className="panel-header compact"><div><h3>Rules of engagement</h3><p>Core enforces these values before any tool request.</p></div><ShieldCheck size={18} /></header>{!scopeAvailable ? <div className="feature-unavailable" role="status"><ShieldAlert size={21} /><div><strong>Scope editing is unavailable</strong><p>This Core build keeps all missions analysis-only.</p></div></div> : <div className="policy-form-body"><div className="resource-form-grid"><label>Allowed CIDRs<textarea rows={3} value={allowedCidrs} placeholder="192.0.2.0/24" onChange={(event) => setAllowedCidrs(event.target.value)} /></label><label>Allowed domains<textarea rows={3} value={allowedDomains} placeholder="app.example.test" onChange={(event) => setAllowedDomains(event.target.value)} /></label></div><label>Allowed URLs<textarea rows={2} value={allowedUrls} placeholder="https://app.example.test/api" onChange={(event) => setAllowedUrls(event.target.value)} /></label><label>Allowed ports<input value={allowedPorts} placeholder="80, 443, 8443" inputMode="numeric" onChange={(event) => setAllowedPorts(event.target.value)} /></label><div className="resource-form-grid"><label>Not before<input type="datetime-local" value={notBefore} onChange={(event) => setNotBefore(event.target.value)} /></label><label>Not after<input type="datetime-local" value={notAfter} onChange={(event) => setNotAfter(event.target.value)} /></label></div><label>Prohibited actions<textarea rows={3} value={prohibitedActions} placeholder="credential use&#10;exploitation&#10;destructive changes" onChange={(event) => setProhibitedActions(event.target.value)} /></label><div className="resource-form-grid"><label>Maximum concurrency<input type="number" min={1} max={10} value={maxConcurrency} onChange={(event) => setMaxConcurrency(Number(event.target.value))} /></label><label className="provider-consent"><input type="checkbox" checked={localOnly} onChange={(event) => setLocalOnly(event.target.checked)} /><span><strong>Local-only processing</strong><small>Prevent cloud providers from receiving engagement data.</small></span></label></div><section className="mission-grants"><header><div><strong>Time-limited mission grants</strong><small>Active scans still require a current matching grant.</small></div><button className="button quiet" type="button" onClick={() => { setGrantError(undefined); setGrantOpen(true); }}>Add grant</button></header>{grants.length ? grants.map((grant, index) => <article key={`${grant.grantedAt}-${index}`}><Clock3 size={14} /><div><strong>{grant.riskClasses.join(", ") || "No risk classes"}</strong><small>{grant.toolNames.join(", ") || "All assigned tools"} · expires {new Intl.DateTimeFormat(undefined, { dateStyle: "medium", timeStyle: "short" }).format(new Date(grant.expiresAt))}</small></div><button className="icon-button subtle" type="button" aria-label="Remove mission grant" onClick={() => setGrants((current) => current.filter((_, itemIndex) => itemIndex !== index))}><X size={13} /></button></article>) : <p>No active grants. Executable requests will remain blocked.</p>}</section><footer><span>Saving creates a revision-safe policy update.</span><button className="button primary" type="submit" disabled={previewMode || saving === "scope"}><Save size={14} /> {saving === "scope" ? "Saving…" : "Save scope"}</button></footer></div>}</form><form className="panel policy-form assignment-form" onSubmit={(event) => void saveAssignment(event)}><header className="panel-header compact"><div><h3>Exact tool assignment</h3><p>Installation alone never grants agent access.</p></div><Wrench size={18} /></header>{!assignmentAvailable ? <div className="feature-unavailable" role="status"><Wrench size={21} /><div><strong>Tool assignments are unavailable</strong><p>Missions remain analysis-only.</p></div></div> : <div className="policy-form-body"><label>Installed pack<select aria-label="Assigned tool pack" value={selectedDigest} disabled={!readyPacks.length} onChange={(event) => { setSelectedDigest(event.target.value); setSelectedTools([]); }}>{readyPacks.length ? readyPacks.map((pack) => <option value={pack.manifestDigest} key={pack.id}>{pack.publisher}/{pack.name} · {pack.version}</option>) : <option value="">No verified packs</option>}</select></label>{selectedDigest && <code className="digest-lock" title={selectedDigest}>{selectedDigest}</code>}<fieldset className="resource-checklist"><legend>Allowed tools</legend>{packTools.length ? packTools.map((tool) => <label key={tool.name}><input type="checkbox" checked={selectedTools.includes(tool.name)} disabled={!tool.available} onChange={(event) => setSelectedTools((current) => event.target.checked ? [...current, tool.name] : current.filter((name) => name !== tool.name))} /><span><strong>{tool.name}</strong><small>{tool.riskClass.replaceAll("_", " ")}{tool.unavailableReason ? ` · ${tool.unavailableReason}` : ""}</small></span></label>) : <p>{readyPacks.length ? "This pack does not expose any tools on the current runner." : "Install and verify a tool pack first."}</p>}</fieldset><label className="provider-consent"><input type="checkbox" checked={assignmentEnabled} disabled={!selectedTools.length} onChange={(event) => setAssignmentEnabled(event.target.checked)} /><span><strong>Enable this assignment</strong><small>Only the exact manifest digest and checked tool names become mission-selectable.</small></span></label><footer><span>Assignment revision {assignment?.revision ?? 0}</span><button className="button primary" type="submit" disabled={previewMode || !selectedDigest || !selectedTools.length || saving === "assignment"}><Save size={14} /> {saving === "assignment" ? "Saving…" : "Save assignment"}</button></footer></div>}</form></div>{grantOpen && <div className="dialog-backdrop"><section className="provider-dialog resource-dialog" role="dialog" aria-modal="true" aria-labelledby="grant-dialog-title"><header><div><small>Explicit authorization</small><h2 id="grant-dialog-title">Add mission grant</h2></div><button className="icon-button subtle" type="button" aria-label="Close mission grant dialog" onClick={() => setGrantOpen(false)}><X size={16} /></button></header><fieldset className="resource-checklist"><legend>Risk classes</legend>{grantRiskOptions.map((risk) => <label key={risk}><input type="checkbox" checked={grantRiskClasses.includes(risk)} onChange={(event) => setGrantRiskClasses((current) => event.target.checked ? [...current, risk] : current.filter((item) => item !== risk))} /><span>{risk.replaceAll("_", " ")}</span></label>)}</fieldset><fieldset className="resource-checklist"><legend>Tools</legend>{grantableToolNames.map((name) => <label key={name}><input type="checkbox" checked={grantTools.includes(name)} onChange={(event) => setGrantTools((current) => event.target.checked ? [...current, name] : current.filter((item) => item !== name))} /><span>{name}</span></label>)}{!grantableToolNames.length && <p>Save and enable an exact tool assignment first.</p>}</fieldset><label>Targets<textarea rows={3} value={grantTargets} placeholder="192.0.2.10&#10;https://app.example.test" onChange={(event) => setGrantTargets(event.target.value)} /></label><label>Expires<input type="datetime-local" value={grantExpires} onChange={(event) => setGrantExpires(event.target.value)} /></label>{grantError && <p className="form-error" role="alert">{grantError}</p>}<p className="provider-dialog-note">The grant is attributed to {activeOperator?.displayName ?? "the active local operator"}. It cannot expand the engagement scope.</p><footer><button className="button secondary" type="button" onClick={() => setGrantOpen(false)}>Cancel</button><button className="button primary" type="button" disabled={!grantExpires || !grantTargets.trim() || !grantRiskClasses.length} onClick={addGrant}>Add grant</button></footer></section></div>}</section>;
}
