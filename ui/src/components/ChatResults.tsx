...
export function ChatResults({api, sessionId, onMessage, onAttach}: {api: ApiClient; sessionId: string; onMessage: (id: string) => void; onAttach: (value: NebulaDraftRequest) => void}) {
  ...
  useEffect(() => {const controller = new AbortController(); setBusy(true); setError(undefined); void api.request<{items: ChatResult[]; next_offset: number | null}>(`chat/sessions/${encodeURIComponent(sessionId)}/results?offset=${offset}`, {signal: controller.signal}).then(value => {setItems(value.items); setNext(value.next_offset);}).catch(e => {void logCaughtDiagnostic("interface.assistant_chat.read_failed", "Assistant chat content could not be loaded.", e, "assistant_chat");if(!controller.signal.aborted) setError(e instanceof Error ? e.message : "Results unavailable.");}).finally(() => {if(!controller.signal.aborted) setBusy(false);}); return () => controller.abort();}, [api, sessionId, offset]);
