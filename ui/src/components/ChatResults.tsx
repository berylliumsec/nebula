...
function ResultImage({api, id}: {api: ApiClient; id: string}) {
  ...
  useEffect(() => {const controller = new AbortController(); let local: string | undefined; void api.fetchChatImagePreview(id, controller.signal).then(blob => {if(!controller.signal.aborted) {local = URL.createObjectURL(blob); setUrl(local);}}).catch(e => {void logCaughtDiagnostic("interface.assistant_chat.preview_failed", "The result preview could not be loaded.", e, "assistant_chat");if(!controller.signal.aborted) setFailed(true);}); return () => {controller.abort(); if(local) URL.revokeObjectURL(local);};}, [api, id]);
...
export function ChatResults({api, sessionId, onMessage, onAttach}: {api: ApiClient; sessionId: string; onMessage: (id: string) => void; onAttach: (value: NebulaDraftRequest) => void}) {
  ...
  useEffect(() => {const controller = new AbortController(); setBusy(true); setError(undefined); void api.request<{items: ChatResult[]; next_offset: number | null}>(`chat/sessions/${encodeURIComponent(sessionId)}/results?offset=${offset}`, {signal: controller.signal}).then(value => {if (!value || !Array.isArray(value.items)) throw new Error("Core did not return a results page"); setItems(value.items); setNext(value.next_offset);}).catch(e => {void logCaughtDiagnostic("interface.assistant_chat.read_failed", "Assistant chat content could not be loaded.", e, "assistant_chat");if(!controller.signal.aborted) setError(e instanceof Error ? e.message : "Results unavailable.");}).finally(() => {if(!controller.signal.aborted) setBusy(false);}); return () => controller.abort();}, [api, sessionId, offset]);
  const run = async (action: () => Promise<void>) => {setError(undefined); setBusy(true); try {await action();} catch(e) { void logCaughtDiagnostic("interface.assistant_chat.operation_failed", "An assistant chat operation failed.", e, "assistant_chat");setError(e instanceof Error ? e.message : "Preview unavailable. The source message remains available.");} finally {setBusy(false);}};
