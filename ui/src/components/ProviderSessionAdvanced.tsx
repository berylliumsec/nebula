...
export function ProviderSessionAdvanced({
  ...
  // Restoring overwrites workspace files, so it asks before acting.
  const restore = async (checkpoint: { id: string; label: string; files: Array<{ path: string }> }) => {
    const count = checkpoint.files.length;
    const approved = await confirm({
      title: "Restore checkpoint?",
      message: <>Restoring <strong>{checkpoint.label}</strong> overwrites the current copies of {count} workspace file{count === 1 ? "" : "s"} with the checkpoint versions.</>,
      confirmLabel: "Restore checkpoint",
      tone: "danger",
    });
    if (!approved) return;
    await act(async () => { await api.restoreChatCheckpoint(checkpoint.id); });
  };
  ...
  return <section className="chat-goal-panel" aria-label="Workspace controls" data-guide="workspace-controls">
    ...
    {checkpoints.length > 0 && <ul>{checkpoints.map(item => <li key={item.id}><strong>{item.label}</strong> <small>{item.files.length} files</small> <button className="button quiet" type="button" disabled={busy} onClick={() => void act(async () => setPreview(await api.previewChatCheckpoint(item.id)))}>Preview</button> <button className="button quiet" type="button" disabled={busy} aria-label={`Restore ${item.label}`} title={`Restore ${item.label}`} onClick={() => void restore(item)}>Restore</button></li>)}</ul>}
