...
export function CodeEditorPanel({ active, api, engagementId, workspacePath, providers = [], harnesses = [], onRun, onOpenTerminal, onCreateFindingDraft, onUseWithAssistant, initialWorkspaceSearch }: CodeEditorPanelProps) {
  ...
  const [referencesRequest, setReferencesRequest] = useState(0);
  ...
  useEffect(() => {
    ...
    setContextualCommands([
      ...
      { id: "editor.definition", label: "Editor: Go to Definition", description: python ? "Navigate to the symbol definition in this bounded Python buffer" : "Definition navigation currently requires an open Python file", keywords: "symbol declaration navigate", shortcut: preferences.keybindings.definition, disabled: !python, run: dispatch("definition") },
      { id: "editor.references", label: "Editor: Find References", description: python ? "List references from bounded Python intelligence" : "Reference search currently requires an open Python file", keywords: "symbol usage navigate", shortcut: preferences.keybindings.references, disabled: !python, run: dispatch("references") },
  ...
  useEffect(() => {
    if (skipBreakpointPersist.current) {
      skipBreakpointPersist.current = false;
      return;
    }
    try {
  ...
  const toggleBreakpoint = useCallback((path: string, line: number) => {
    setBreakpoints((current) => {
      ...
      return { ...current, [path]: [...selected].sort((left, right) => left - right) };
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const runEditorCommand = (command: string): boolean => {
      ...
      else if (command === "definition" && buffer?.filePath.endsWith(".py")) setDefinitionRequest((request) => request + 1);
      else if (command === "references" && buffer?.filePath.endsWith(".py")) setReferencesRequest((request) => request + 1);
      ...
      {buffer ? <>
        ...
        <header className="code-editor-toolbar">
          ...
          <div className="code-editor-actions">
            ...
            <button className="button quiet code-editor-tools-trigger" type="button" aria-label="More editor tools" aria-expanded={editorToolsOpen} aria-controls="code-editor-tools" title="More editor tools" onClick={() => setEditorToolsOpen(open => !open)}><MoreHorizontal size={18} /></button><div id="code-editor-tools" className={`code-editor-secondary-actions${editorToolsOpen ? " open" : ""}`} aria-label="Editor tools" onClick={() => setEditorToolsOpen(false)}><button className="button quiet quiet-icon-action" type="button" title={`Quick open (${preferences.keybindings.quickOpen})`} onClick={() => setWorkspaceSearchMode("files")} aria-label="Open"><Search size={18} aria-hidden="true" /></button><button className="button quiet quiet-icon-action" type="button" title={`Search workspace (${preferences.keybindings.workspaceSearch})`} onClick={() => setWorkspaceSearchMode("text")} aria-label="Search"><TextSearch size={18} aria-hidden="true" /></button><button className="button quiet quiet-icon-action" type="button" onClick={() => setFindRequest((request) => request + 1)} aria-label="Find" title="Find"><Search size={18} aria-hidden="true" /></button><button className="button quiet quiet-icon-action" type="button" disabled={!onRun} onClick={() => setTasksOpen(true)} aria-label="Tasks" title="Tasks"><ListTodo size={18} aria-hidden="true" /></button><button className="button quiet quiet-icon-action" type="button" disabled={!buffer.filePath.endsWith(".py")} title="Python Problems (Ctrl+Shift+M)" onClick={() => setProblemsRequest((request) => request + 1)} aria-label="Problems"><ListChecks size={18} aria-hidden="true" /></button><button className="button quiet quiet-icon-action" type="button" disabled={!buffer.filePath.endsWith(".py")} title="Format Python document (Shift+Alt+F)" onClick={() => setFormatRequest((request) => request + 1)} aria-label="Format"><Paintbrush size={18} aria-hidden="true" /></button><button className="button quiet quiet-icon-action" type="button" disabled={!buffer.filePath.endsWith(".py")} title="Rename Python symbol (F2)" onClick={() => setRenameRequest((request) => request + 1)} aria-label="Rename"><PencilLine size={18} aria-hidden="true" /></button><button className="button quiet" type="button" disabled={buffers.length < 2} title={buffers.length < 2 ? "Open another file before splitting" : preferences.keybindings.splitEditor} onClick={secondaryBuffer ? closeSplit : splitEditor}><Columns2 size={14} /> {secondaryBuffer ? "Unsplit" : "Split"}</button><button className="button quiet" type="button" aria-label="Editor settings" onClick={() => setPreferencesOpen(true)}><Settings2 size={14} /></button><button className="button quiet quiet-icon-action" type="button" disabled={!suggestionRuntimes.length} title={!suggestionRuntimes.length ? "Enable a runtime with explicit code-suggestion capability" : undefined} onClick={openSuggestion} aria-label="Suggest"><Sparkles size={18} aria-hidden="true" /></button></div>
