...
export function MissionPromotionDialog({ run, summary }: { run: AgentRunSummary; summary: string }) {
  ...
  return <>
    ...
    {open && createPortal(<ModalSurface as="form" noValidate className="provider-dialog resource-dialog mission-promotion-dialog" labelledBy="mission-promotion-title" onClose={() => { if (!saving) setOpen(false); }} onSubmit={(event) => void submit(event)}>
      <header><div><small>Operator-reviewed promotion</small><h2 id="mission-promotion-title">Create from Mission result</h2></div><button className="icon-button subtle" type="button" aria-label="Close promotion dialog" disabled={saving} onClick={() => setOpen(false)}><X size={17} /></button></header>
      ...
      <footer><button className="button secondary" type="button" disabled={saving} onClick={() => setOpen(false)}>Cancel</button><button className="button primary" type="submit" disabled={saving || !title.trim() || !body.trim()}>{saving ? "Creating…" : kind === "finding" ? "Create candidate" : "Create report draft"}</button></footer>
