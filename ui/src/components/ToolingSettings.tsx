...
export function AutomationRuntimeSettings() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [nextRuntime, nextVpnProfiles, nextProjectPolicy] = await Promise.all([
        api.getAutomationRuntime(undefined, engagement?.id),
  ...
  const uploadVpn = async (event: FormEvent) => {
    ...
    try {
      ...
      if (engagement && projectPolicy && projectPolicy.executionMode !== "host") {
        try {
  ...
  return <section className="settings-section" id="automation-runtime-settings">
    <div className="section-heading"><div><h2>Automation runtime</h2><p>Agent commands use the project’s selected Host or Docker mode and ordinary binaries on PATH.</p></div><button className="button secondary" type="button" disabled={loading || preparing} onClick={() => void load()}><RefreshCw size={14} /> Refresh</button></div>
    {error && <DiagnosticErrorNotice error={error} fallback="The automation runtime could not be inspected." compact />}
    {projectPolicy?.executionMode === "host" ? <section className="panel runner-status"><header className="panel-header compact"><div><h3>Host mode</h3><p>{runtime?.detail ?? "Commands run as the Nebula server OS user."}</p></div><TerminalSquare size={18} /></header><p>Host commands can access permitted host folders and mounted devices. Docker images and container VPN routes are not used. Select Docker mode in Project execution policy to restore container isolation.</p></section> : !runtime ? <div className="feature-unavailable" role="status"><AlertTriangle size={22} /><div><strong>{loading ? "Checking runtime…" : "Runtime unavailable"}</strong><p>Configure a pinned automation image and a verified local runner.</p></div></div> :
