...
function fileBase64(file: File): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    reader.onerror = () => reject(reader.error ?? new Error("Could not read the tool-pack bundle."));
...
export function ToolPackSettings() {
  ...
  const load = useCallback(async () => {
    ...
    if (failure?.status === "rejected") setError(failure.reason instanceof Error ? failure.reason.message : "Could not load tool-pack status.");
    setLoading(false);
  }, [api, coreState]);
  ...
  const action = async (id: string, operation: () => Promise<unknown>) => {
    ...
    try {
      ...
      setError(actionError instanceof Error ? actionError.message : "The tool-pack operation failed.");
      return false;
    } finally {
      setBusy(undefined);
    }
  ...
  return (
    <section className="settings-section" id="tool-pack-settings">
      <div className="section-heading"><div><h2>Tool packs</h2><p>Signed, digest-pinned capabilities installed in the selected external container runtime.</p></div><button className="button secondary" type="button" disabled={loading || previewMode} onClick={() => void load()}><RefreshCw size={14} /> Refresh</button></div>
      {error && <div className="knowledge-status error" role="alert">{error}</div>}
      {!readyRunner && <div className="knowledge-status warning" role="status"><AlertTriangle size={15} /> Configure a runner before installing a tool pack. Existing tools remain unavailable to missions.</div>}
