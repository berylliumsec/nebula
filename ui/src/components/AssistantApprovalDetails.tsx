import { mcpToolLabel } from "../mcpTools";
...
export function AssistantApprovalDetails({request}: {request: Record<string, unknown>}) {
  ...
  const toolName = exact.tool_name ?? exact.name;
  // An MCP tool is named for the routing runtime, not for the person deciding.
  const label = (typeof exact.display_name === "string" && exact.display_name)
    || (typeof toolName === "string" ? mcpToolLabel(toolName) : undefined)
    || String(toolName ?? "Requested action");
  return <><p>{label}</p>{typeof resource === "string" && <p className="chat-approval-resource" title={resource}>Resource: {resource}</p>}<p>Approving allows this action to run with the arguments below.</p><details><summary>Exact request</summary><pre tabIndex={0} role="region" aria-label="Exact request JSON">{JSON.stringify(exact, null, 2)}</pre></details></>;
}
