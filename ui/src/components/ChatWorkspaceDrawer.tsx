...
export function ChatWorkspaceDrawer({children, tab, onTab, onClose, overlay = false}: {overlay?: boolean; children: ReactNode; tab: string; onTab: (tab: "context" | "results") => void; onClose: () => void}) {
  ...
  const content = <><header><strong id={id}>Conversation details</strong><IconAction icon={X} label="Close details" onClick={onClose} /></header><nav aria-label="Conversation detail views">{(["context", "results"] as const).map(item => <button className="button quiet" aria-current={tab === item ? "page" : undefined} key={item} onClick={() => onTab(item)}>{item === "context" ? "Context" : "Results"}</button>)}</nav>{children}</>;
