...
export function ChatWorkspaceDrawer({children, tab, onTab, onClose}: {children: ReactNode; tab: string; onTab: (tab: "context" | "results") => void; onClose: () => void}) {
  ...
  return mobile ? createPortal(<ModalSurface as="section" className="session-inspector assistant-drawer" labelledBy={id} onClose={onClose}>{content}</ModalSurface>, document.body) : <aside className="session-inspector assistant-drawer" aria-label="Session inspector">{content}</aside>;
}
