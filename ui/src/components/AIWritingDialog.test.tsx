...
describe("AIWritingDialog", () => {
  ...
  it("reconciles a refreshed harness model catalog before retrying", async () => {
    const user = userEvent.setup();
    const staleHarness = {
      ...harness,
      id: "grok-harness",
    ...
    const transformWriting = vi.fn()
      .mockRejectedValueOnce(new Error("model 'grok-build' is not supported by the harness"))
      .mockResolvedValueOnce({
        ...response,
        provenance: { ...response.provenance, providerProfileId: staleHarness.id, model: "grok-4.6" },
      });
    const props = {
      api: { transformWriting } as unknown as ApiClient,
      engagementId: "engagement-1",
      providers: [],
      purpose: "report_summary" as const,
      title: "Draft executive summary with AI",
      description: "Draft from selected content.",
      sourceLabel: "Report context",
      sourceText: "One validated medium finding.",
      initialInstruction: "Make it concise.",
    ...
    const view = render(<AIWritingDialog {...props} harnesses={[staleHarness]} />);

    await user.click(screen.getByRole("button", { name: "Generate draft" }));
    expect(await screen.findByText("model 'grok-build' is not supported by the harness", { exact: false })).toBeVisible();

    view.rerender(<AIWritingDialog {...props} harnesses={[refreshedHarness]} />);
    await waitFor(() => expect(screen.getByRole("combobox", { name: "AI writing model" })).toHaveValue("grok-4.6"));
    expect(screen.queryByText("model 'grok-build' is not supported by the harness", { exact: false })).not.toBeInTheDocument();
    await user.click(screen.getByRole("button", { name: "Generate draft" }));

    await waitFor(() => expect(transformWriting).toHaveBeenLastCalledWith(
      expect.objectContaining({
        backendKind: "harness",
        harnessProfileId: "grok-harness",
        model: "grok-4.6",
      }),
      expect.any(AbortSignal),
    ));
    expect(await screen.findByRole("textbox", { name: "AI writing draft" })).toHaveValue("Generated report prose.");
  });
});
