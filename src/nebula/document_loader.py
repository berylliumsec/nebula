...
from PyQt6.QtWidgets import (QComboBox, QDialog, QFileDialog, QHBoxLayout,
                             QLineEdit, QMessageBox, QPushButton, QVBoxLayout, QProgressDialog)
...
class DocumentLoaderDialog(QDialog):
    def __init__(self, vector_db, parent=None):
        logger.debug("Initializing DocumentLoaderDialog with manager: %s, parent: %s", vector_db, parent)
    ...
    def init_ui(self):
        ...
        self.type_combo.addItems(["url", "pdf", "text", "csv", "json", "jsonl", "directory"])
    ...
    def load_document(self):
        ...
        logger.info("Load document triggered with source: '%s' and source_type: '%s'", source, source_type)
        if not source:
            QMessageBox.warning(self, "Input Error", "Please enter a URL or file/folder path.")
            return
        try:
            docs = self.vector_db.load_documents(source, source_type=source_type)
            # Create a progress dialog.
            progress_dialog = QProgressDialog("Adding documents...", "Cancel", 0, 100, self)
            ...
            worker.signals.finished.connect(lambda: (
                ...
                QMessageBox.information(self, "Documents Loaded",
            ...
            progress_dialog.canceled.connect(lambda: worker.cancel() if hasattr(worker, 'cancel') else None)
            QThreadPool.globalInstance().start(worker)
        except Exception as e:
            logger.exception("Error loading documents from source: '%s', source_type: '%s'", source, source_type)
