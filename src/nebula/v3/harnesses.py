...
def _harness_developer_instructions(
    ...
    *,
    ...
    return (
        ...
        "work; its supplied tool description identifies Docker or Host execution. "
        "The vendor launch directory is private scratch, not the project workspace. "
        "Use the supplied workspace search and read tools to inspect project files, or "
        "the supplied command tool with cwd set to dot to inspect the project folder. "
        "These tools resolve the selected project folder. Never infer that the "
        "project is empty from the vendor launch directory. "
        ...
        "or other assigned capabilities. Questions about which knowledge sources are "
        "available are resource requests, not abstract capability questions; call knowledge.list "
    ...
    async def open(self, request: AdapterOpenRequest) -> HarnessConnection:
        ...
        try:
            ...
            if request.gateway_config:
                mcp_servers = []
                for name, config in request.gateway_config.items():
                    if config.get("transport") != McpTransport.STDIO.value:
                        raise HarnessConfigurationError(
                            "Grok gateway requires a stdio MCP transport"
                        )
                    mcp_servers.append(
                        {
                            "name": name,
    ...
    async def _connection(
        ...
        def gateway_catalog(params: dict[str, Any]) -> dict[str, Any]:
            ...
            return {
                **catalog,
                "tools": [
                    {**tool, "name": _portable_gateway_tool_name(tool["name"])}
                    for tool in catalog["tools"]
                ],
            }

        def gateway_call(name: str, arguments: dict[str, Any]) -> Any:
            if portable_names:
                matches = [
                    tool["name"]
                ...
                if len(matches) != 1:
                    raise HarnessConfigurationError("Unknown or ambiguous gateway tool")
                name = matches[0]
            return self._gateway_call(session, name, arguments)
