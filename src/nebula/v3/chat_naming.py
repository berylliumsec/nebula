...
_GREETING = re.compile(r"\s*(?:hi|hello|hey|greetings|good (?:morning|afternoon|evening)|thanks|thank you)[!.\s]*", re.I)

def substantive_prompt(prompts: list[str]) -> str:
    return next((text for text in prompts if text.strip() and not _GREETING.fullmatch(text)), "")
