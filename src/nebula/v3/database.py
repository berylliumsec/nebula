...
class ApplicationModelOutboxRow(Base):
    __tablename__ = "application_model_outbox"
    __table_args__ = (
        UniqueConstraint("model_session_id", "source_kind", "source_id", "source_revision", "adapter_version", name="uq_application_model_source"),
        Index("ix_application_model_pending", "model_session_id", "status", "created_at"),
    )
    id: Mapped[str] = mapped_column(String(64), primary_key=True)
    engagement_id: Mapped[str] = mapped_column(String(200), nullable=False)
    model_session_id: Mapped[str] = mapped_column(String(200), nullable=False)
    source_kind: Mapped[str] = mapped_column(String(80), nullable=False)
    source_id: Mapped[str] = mapped_column(String(200), nullable=False)
    source_revision: Mapped[int] = mapped_column(Integer, nullable=False)
    adapter_version: Mapped[str] = mapped_column(String(20), nullable=False, default="1")
    payload: Mapped[dict[str, Any]] = mapped_column(JSON, nullable=False)
