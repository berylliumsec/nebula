...
def workspace_router(store: NebulaStore) -> APIRouter:
    ...
    def search(
        ...
        statement = statement.where(exists(select(visible_chat.id).where(
            visible_chat.kind == "chat_sessions",
            visible_chat.id == EntityRow.payload["session_id"].as_string(),
            func.coalesce(visible_chat.payload["metadata"]["temporary_assistant"].as_boolean(), False).is_(False),
        )))
        if session_id:
            ...
            statement = statement.where(
                exists(
                    select(mark.id).where(
                        mark.kind == "chat_bookmarks",
                        mark.engagement_id == project_id,
                        mark.payload["message_id"].as_string() == EntityRow.id,
                        mark.payload["active"].as_boolean().is_(True),
                    )
                )
            )
        statement = (
