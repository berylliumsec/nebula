...
def results_router(store, artifacts):
    ...
    def source_preview(project_id: str, source_id: str):
        ...
        return {
            "text": selected,
            "truncated": len(text) > len(selected),
    ...
    def results(
        ...
        for message in messages[:limit]:
            ...
            for index, match in enumerate(
                ...
                items.append(
                    {
                        "id": f"{message.id}-code-{index}",
                        "message_id": message.id,
                        "kind": "code",
                        "label": match[1] or "Code excerpt",
                        "text": match[2],
                        "artifact_id": None,
                    }
                )
            ...
            if turn_id and artifacts is not None:
                with store.database.session() as database:
                    diffs = [
                        ...
                        for row in database.scalars(
                            select(EntityRow).where(
                                EntityRow.kind == "artifacts",
                                EntityRow.engagement_id == session.engagement_id,
                                EntityRow.payload["source"].as_string()
                                == "harness-file-diff",
                                EntityRow.payload["metadata"][
                                    "harness_turn_id"
                                ].as_string()
                                == turn_id,
                            )
                    ...
                    items.append(
                        {
                            ...
                            "message_id": message.id,
                            "kind": "file_change",
                            "label": "Recorded file changes (bounded preview)",
                            "text": preview,
                            "artifact_id": diff.id,
                        }
                    )
            for citation in message.citations:
                items.append(
                    {
                        "id": f"{message.id}-{citation.chunk_id}",
                        "message_id": message.id,
                        "kind": "citation",
                        "label": citation.name,
                    ...
                    items.append(
                        {
                            ...
                            "artifact_id": call.result_artifact_id,
                            "status": call.status,
                            "text": call.error,
                        }
                    )
    ...
    def context_sources(session_id: str, offset: int = Query(default=0, ge=0)):
        ...
        return {
            ...
            if session.backend.value == "provider"
            else "Harness internal instructions are not fully exposed to Nebula.",
        }
