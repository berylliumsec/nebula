...
logger = setup_logging(
    log_file=f"{constants.SYSTEM_LOGS_DIR}/memory.log"
)
...
class ConversationMemory:
    def __init__(self, max_tokens=20000, file_path = None, encoding_name="cl100k_base"):
        ...
        self.history = []  # List of messages: each message is a dict with keys "role" and "content"
        # Initialize the tokenizer encoder from tiktoken for accurate token counting
        self.encoder = tiktoken.get_encoding(encoding_name)
        logger.info(f"Initializing ConversationMemory with max_tokens={max_tokens}, file_path={self.file_path}")
        self.load()
    ...
    def trim_memory(self):
        ...
        while total_tokens > self.max_tokens and self.history:
            ...
            logger.info(f"Trimming memory: removed message with role={removed['role']} containing {removed_tokens} tokens")
            total_tokens = sum(self.estimate_tokens(msg["content"]) for msg in self.history)
    ...
    def load(self):
        ...
        if os.path.exists(self.file_path):
            try:
                ...
                logger.info(f"Successfully loaded conversation memory from {self.file_path}")
            except Exception as e:
                logger.error(f"Error loading conversation memory: {e}")
                self.history = []
        else:
            logger.info(f"No existing conversation memory found at {self.file_path}. Starting fresh.")
